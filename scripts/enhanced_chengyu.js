...
console.log(`读取成语数据: ${enhancedDataPath}`);
let enhancedData = {};

try {
  enhancedData = require(enhancedDataPath);
  console.log(`成功读取现有数据，包含 ${Object.keys(enhancedData).length} 个成语`);
} catch (error) {
...
function addNewChengyu(chengyu, bestAnswer, candidates, explanation, score = 100) {
  if (enhancedData[chengyu]) {
    console.log(`成语 "${chengyu}" 已存在，将更新其数据`);
  } else {
    console.log(`添加新成语: "${chengyu}"`);
  }
...
console.log(`当前共有 ${Object.keys(enhancedData).length} 个成语`);

// 使用说明
console.log('\n使用说明:');
console.log('1. 打开此文件');
console.log('2. 找到添加新成语的示例代码部分（被注释掉的addNewChengyu调用）');
console.log('3. 取消注释并修改参数来添加新成语');
